...
useSeoMeta({
	title: '图片水印 - 工具箱',
	description: '上传图片，添加文字水印。可调节位置、大小、透明度。纯浏览器端，数据不上传。',
})
...
function drawText(ctx: CanvasRenderingContext2D) {
	...
	const pad = Math.max(tw * 0.3, 40)

	let x: number, y: number
	switch (position.value) {
		case 'tl': x = pad; y = pad + th / 2; break
		case 'tr': x = canvasEl.value!.width - pad; y = pad + th / 2; break
		case 'bl': x = pad; y = canvasEl.value!.height - pad - th / 2; break
		case 'br': x = canvasEl.value!.width - pad; y = canvasEl.value!.height - pad - th / 2; break
...
<div class="tool-page">
	...
	<h1>图片水印</h1>
	<p class="subtitle">添加文字水印，纯浏览器端处理，不涉及图片上传。</p>
