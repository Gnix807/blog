		...
		<div class="controls" v-if="previewUrl">
			<div class="control-row">
				...
				<div class="format-btns">
					<button v-for="opt in formatOptions" :key="opt.value" :class="{ active: format === opt.value }" @click="format = opt.value; convertImage(previewUrl)">
						{{ opt.label }}
					</button>
				</div>
			</div>
			<div class="control-row" v-if="format !== 'image/png'">
				...
				<input type="range" min="0.1" max="1" step="0.05" v-model.number="quality" class="slider" @change="convertImage(previewUrl)">
			</div>
...
.controls {
	...
	.result-info {
		display: flex; align-items: center; gap: 0.8em; padding: 0.8em;
		border-radius: 0.5em; background-color: var(--c-bg-soft);
		.size-item { text-align: center; .size-label { display: block; font-size: 0.75em; color: var(--c-text-3); } .size-value { font-weight: 600; font-size: 0.95em; color: var(--c-text); &.converted { color: var(--c-primary); } } }
